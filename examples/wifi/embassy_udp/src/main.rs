...
//!
//! Connects to a WPA2-Personal Wi-Fi network, waits for DHCP, sends `banana` to
//! a UDP socket on another machine, and prints any replies. The destination
//! machine must be reachable from the ESP's network.
//!
...
//! ```
//!
//! Use `--chip=esp32s3` instead for an ESP32-S3. The `xtask` selects the
//! appropriate RISC-V or Xtensa target and toolchain.
