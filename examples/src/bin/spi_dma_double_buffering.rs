//! SPI loopback using multiple DMA buffers
//!
...
fn main() -> ! {
    ...
    let (mut dma_rx_buf, mut dma_tx_buf) = {
    ...
    let (dma_rx_buf1, mut dma_tx_buf1) = {
        let (rx_buffer, rx_descriptors, tx_buffer, tx_descriptors) = dma_buffers!(3200);
        let dma_rx_buf = DmaRxBuf::new(rx_descriptors, rx_buffer).unwrap();
        let dma_tx_buf = DmaTxBuf::new(tx_descriptors, tx_buffer).unwrap();
        (dma_rx_buf, dma_tx_buf)
    ...
    for (i, v) in dma_tx_buf1.as_mut_slice().iter_mut().enumerate() {
        *v = (i % 255) as u8;
    }

    let mut i = 0;

    let mut transfer = spi.dma_transfer(dma_rx_buf1, dma_tx_buf1)
        .map_err(|e| e.0)
        .unwrap();
