//! Battery service peripheral with BLE modem sleep enabled.
//!
//! Select the clock that runs the controller while it sleeps with `BLE_LP_CLK`. The radio
//! wake lock still prevents SoC light sleep.
...
async fn main(_s: Spawner) {
    ...
    let connector = BleConnector::new(
        bluetooth,
        esp_radio::ble::Config::default().with_modem_sleep(true),
    )
    .unwrap();
