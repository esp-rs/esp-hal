...
impl ClockTreeNodeType for Generic {
    ...
    fn validate_configures_expr(
        ...
        let (param, v) = if let Some(property) = expr.effect().property.as_ref() {
            self.params
                ...
                .ok_or_else(|| anyhow::anyhow!("Divider does not have property `{}`", property))?
        } else {
            anyhow::ensure!(
                self.params.len() == 1,
                "Configuring dividers with multiple parameters requires specifying a property",
            );
    ...
    fn apply_configuration(
        ...
        // Initially we support the two simple cases:
        // - This node is a simple mux
        // - This node is a simple one-parameter divider
        assert!(self.params.len() == 1);
            ...
            match v {
                NodeParameter::Value(values) => {
                    ...
                    let cfg_expr_code = ExprCompiler::new(&variables)
                        .compile_right_hand_expression(&expr.source, &effect.value, tree);
    ...
    fn node_frequency_impl(
        ...
        self.output.to_rust(variables, tree)
    }
...
impl Generic {
    ...
    fn impl_request_upstream(
        ...
        self.impl_mux_operation(instance, tree, config_var, |node| node.request_fn_name())
    }
    ...
    fn impl_release_upstream(
        ...
        self.impl_mux_operation(instance, tree, config_var, |node| node.release_fn_name())
    }
