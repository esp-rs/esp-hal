...
/// Wake the controller before a host->controller send if modem sleep put it to sleep.
/// Mirrors bt.c's async_wakeup_request(VHCI): flag the request, and if the controller
/// is not active, block-wake it. Without this, an HCI send to a sleeping controller hangs.
pub(crate) fn hci_wakeup_request() {
    unsafe extern "C" {
...
pub(crate) fn hci_wakeup_request_end() {
    unsafe extern "C" {
        fn btdm_in_wakeup_requesting_set(set: bool);
    }
    unsafe {
