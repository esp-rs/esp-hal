...
pub(crate) fn mutex_lock(mutex: *mut c_void) -> i32 {
    mutex_lock_with_timeout(mutex, OSI_FUNCS_TIME_BLOCKING)
}

pub(crate) fn mutex_lock_with_timeout(mutex: *mut c_void, tick: u32) -> i32 {
    let ptr = unwrap!(SemaphorePtr::new(mutex.cast()), "mutex is null");
...
pub(crate) fn mutex_try_lock(mutex: *mut c_void) -> bool {
    let ptr = unwrap!(SemaphorePtr::new(mutex.cast()), "mutex is null");
