...
//! ## Network stack
//!
//! This module establishes and controls the IEEE 802.11 link. It does not implement IP, DHCP,
//! DNS, TCP, or UDP. [`WifiController`] configures the radio, scans for access points, and manages
//! the connection. [`Interface`] exchanges network frames; pass it to [`embassy-net`] to obtain an
//! IP address and create TCP or UDP sockets.
//!
//! A station-mode application typically:
//!
//! 1. Allocates a heap and starts the preemptive scheduler as described in the [crate-level quick
//!    start](crate#quick-start).
//! 2. Creates a [`sta::StationConfig`] and [`WifiController`].
...
//!    executor task.
//! 5. Connects the controller and waits for the Embassy network configuration.
//! 6. Creates an Embassy TCP or UDP socket to exchange application data.
//!
//! See the [`embassy_udp`] example for the complete setup and commands for ESP32-C3 and ESP32-S3.
//! The [`embassy_dhcp`] example demonstrates DNS and HTTP over TCP.
//!
