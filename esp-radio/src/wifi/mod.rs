...
impl WifiEventData {
    pub(crate) unsafe fn try_from_raw(
        ...
        match event_id {
            event if event == crate::sys::include::wifi_event_t_WIFI_EVENT_SCAN_DONE as i32 => {
                let data = unsafe { &*(event_data as *const wifi_event_sta_scan_done_t) };
                Some(WifiEventData::ScanDone {
                    number: data.number,
                })
            }
            event if event == crate::sys::include::wifi_event_t_WIFI_EVENT_AP_START as i32 => {
                Some(WifiEventData::AccessPointStart)
            }
            event if event == crate::sys::include::wifi_event_t_WIFI_EVENT_AP_STOP as i32 => {
                Some(WifiEventData::AccessPointStop)
            }

            event if event == crate::sys::include::wifi_event_t_WIFI_EVENT_STA_START as i32 => {
                Some(WifiEventData::StationStart)
            }
            event if event == crate::sys::include::wifi_event_t_WIFI_EVENT_STA_CONNECTED as i32 => {
                let data = unsafe { &*(event_data as *const wifi_event_sta_connected_t) };
                Some(WifiEventData::StationConnected {
            ...
            event
                if event
                    == crate::sys::include::wifi_event_t_WIFI_EVENT_STA_DISCONNECTED as i32 =>
            {
                let data = unsafe { &*(event_data as *const wifi_event_sta_disconnected_t) };
                Some(WifiEventData::StationDisconnected {
                    reason: data.reason,
                    ssid: core::str::from_utf8(&data.ssid[..data.ssid_len as usize])
                        .unwrap_or_default()
                        .to_string(),
...
impl WifiController<'_> {
    ...
    pub async fn start_async(&mut self) -> Result<(), WifiError> {
        ...
        let mut awaited_event_count = 0;
        let mode = self.mode()?;
        if mode.is_access_point() {
            awaited_event_count += 1;
            events.push(WifiEvent::AccessPointStart);
        }
        if mode.is_station() {
            awaited_event_count += 1;
            events.push(WifiEvent::StationStart);
        }
        ...
        let _ = self
            .handle_events_async(
                ...
                |this| this.start(),
                |event| {
                    if let WifiEventData::AccessPointStart = event {
                        count += 1;
                    } else if let WifiEventData::StationStart = event {
                        count += 1;
                    }

                    if count == awaited_event_count {
                        Some(Ok(()))
                    } else {
                        None
                    }
    ...
    pub async fn stop_async(&mut self) -> Result<(), WifiError> {
        ...
        if mode.is_access_point() {
            awaited_event_count += 1;
            events.push(WifiEvent::AccessPointStop);
        }
        if mode.is_station() {
            awaited_event_count += 1;
            events.push(WifiEvent::StationStop);
        }
    ...
    pub async fn wait_for_access_point_stopped(&mut self) {
        let _ = self
            .handle_events_async(
                ...
                |_| Ok(()),
                |event| {
                    if let WifiEventData::AccessPointStop = event {
                        Some(Ok(()))
                    } else {
                        None
                    }
    ...
    async fn handle_events_async<T>(
        &mut self,
        events: &[WifiEvent],
        action: impl FnOnce(&mut Self) -> Result<(), WifiError>,
        mut waiter: impl FnMut(WifiEventData) -> Option<Result<T, WifiError>>,
    ) -> Result<T, WifiError> {
        SUBSCRIBED_EVENTS.with(|subscribed| subscribed.extend_from_slice(events));

        let mut subscriber = WIFI_EVENTS
            .subscriber()
            .expect("Unable to subscribe to events");
        ...
        if let Err(err) = err {
            SUBSCRIBED_EVENTS.with(|subscribed| {
                for event in events {
                    if let Some(position) = events.iter().position(|item| item == event) {
        ...
        SUBSCRIBED_EVENTS.with(|subscribed| {
            for event in events {
                if let Some(position) = subscribed.iter().position(|item| item == event) {
                    subscribed.remove(position);
                }
            }
        });
