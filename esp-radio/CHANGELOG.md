...
- `Interface::station()`, `Interface::try_station()`, `Interface::access_point()`, and `Interface::try_access_point()` singleton constructors (#5376)
- `WifiController::esp_now()` and `WifiController::sniffer()` methods to create ESP-NOW and sniffer instances tied to the controller's lifetime (#5376)
- `Drop` impl for `Sniffer` that disables promiscuous mode and unregisters the callback (#5376)

### Changed

- `wifi::new()` now returns `WifiController` instead of `(WifiController, Interfaces)` (#5376)
- `Interface` no longer has a lifetime parameter and is no longer `Clone`/`Copy` (singleton semantics) (#5376)
...
- Removed the `Interfaces` struct; interfaces and sub-peripherals are now created independently (#5376)
