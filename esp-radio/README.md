...
The minimal [`embassy_udp`] example shows the complete Station mode data path.
It connects to an access point, obtains an IPv4 address through DHCP, creates an
`embassy_net::udp::UdpSocket`, and sends `b"banana"` to another machine. The
Wi-Fi [examples guide] includes commands for the ESP32-C3 and ESP32-S3 and
points to examples for TCP, HTTP, and Access Point mode.
