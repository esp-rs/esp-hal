...
fn regi2c_enable_block(block: u8) {
    ...
    unsafe{
        (I2C_MST_ANA_CONF2_REG as *mut u32).write_volatile(         // (1 << 18)
...
// fn reg_set_field(reg : u32, s: u32, v: u32) {
//     unsafe {
//         (reg as *mut u32).write_volatile(((reg as *mut u32).read_volatile() & !(v << s)) | (v & (v << s)));
//     }
// }
