...
//! 
//! ⚠️ NOTE: notice that `psram` and `radio` are marked with `false` in the `peripherals!` macro.
//! Basically, that means that there's no real peripheral (no `PSRAM` nor `RADIO` peripheral in the PACs) but we're
