...
// TODO: Maybe make Sha Generic (Sha<Mode>) in order to allow for better compiler optimizations? 
// (Requires complex const generics which isn't stable yet)

#[cfg(esp32)]
impl Sha {
    pub fn new(sha: SHA, mode: ShaMode) -> Self {
        // Setup SHA Mode
        Self { sha, mode, first_run: true, cursor: 0, finished: false }
    }
    ...
    {   
        ...
        unsafe {
            // Read SHA1=Text[0:4] | SHA256=Text[0:8] | SHA384=Text[0:11] | SHA512=Text[0:15]
            // TODO: limit read len to digest size
            core::ptr::copy_nonoverlapping(self.sha.text_.as_ptr() as *const u8, output.as_mut_ptr(), output.len());
        }
        Ok(())
    }
...
impl Sha {
    pub fn new(sha: SHA, mode: ShaMode) -> Self {
        ...
        Self { sha, mode, cursor: 0, first_run: true, finished: false }
    }
