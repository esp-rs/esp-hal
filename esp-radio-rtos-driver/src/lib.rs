...
unsafe extern "Rust" {
    ...
    fn esp_rtos_schedule_task_deletion(task_handle: ThreadPtr);
...
macro_rules! register_scheduler_implementation {
    ($vis:vis static $driver:ident: $t: ty = $val:expr) => {
        ...
        #[unsafe(no_mangle)]
        #[inline]
        fn esp_rtos_schedule_task_deletion(task_handle: $crate::ThreadPtr) {
...
pub trait SchedulerImplementation: Send + Sync + 'static {
    ...
    /// This function sets the priority of the current task.
    ///
    /// # Safety
    ///
    /// The task pointer must be valid and point to a task that was created using
...
/// [`current_task`].
#[inline]
pub unsafe fn schedule_task_deletion(task_handle: ThreadPtr) {
