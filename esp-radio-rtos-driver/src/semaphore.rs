...
impl SemaphoreHandle {
    ...
    /// Decrements the semaphore's counter.
    ///
    /// If a timeout is given, this function blocks until either a semaphore could be taken, or
    /// the timeout has been reached. If no timeout is given, this function blocks until
    /// the operation succeeds.
    ///
    /// This function returns `true` if the semaphore was taken, `false` if the timeout was
    /// reached.
    #[inline]
    ...
    /// Decrements the semaphore's counter.
    ///
    /// If a deadline is given, this function blocks until either a semaphore could be taken, or
    /// the deadline has been reached. If no deadline is given, this function blocks
    ...
    ///
    /// If the flag is `Some`, the implementation may set it to `true` to request a context
    /// switch.
    #[inline]
    ...
    /// If the counter is zero, this function returns `false`.
    ///
    /// If a higher priority task is woken up by this operation, the `higher_prio_task_waken`
    /// flag is set to `true`.
    #[inline]
...
mod implementation {
    ...
    impl SemaphoreInner {
            ...
            match self {
                ...
                SemaphoreInner::Mutex {
                    ...
                    if let Some(owner) = owner {
                        if *owner == current_thread && *recursive {
                            *lock_counter += 1;
                            true
                        } else {
                            unsafe {
                                // We can't lock the mutex. Make sure the mutex holder has a high
                                // enough priority to avoid priority inversion.
                                let current_priority = task_priority(current_thread);
                                if task_priority(*owner) < current_priority {
                                    set_task_priority(*owner, current_priority);
                                }
                            }
                            false
                        }
                        ...
                        *lock_counter += 1;
                        *original_priority = unsafe { task_priority(current_thread) };
                        true
                    }
                }
            }
        ...
        fn try_take_from_isr(&mut self) -> bool {
            match self {
                ...
                SemaphoreInner::Mutex {
                    ...
                    // priority inheritance and we have to conjure up an owner.
        ...
        fn wait_with_deadline(&mut self, deadline: Option<u64>) {
            match self {
                Self::Counting { waiting, .. } => waiting.wait_until(deadline),
                Self::Mutex { waiting, .. } => waiting.wait_until(deadline),
            }
        }

        fn notify(&mut self) {
            match self {
                Self::Counting { waiting, .. } => waiting.notify(),
                Self::Mutex { waiting, .. } => waiting.notify(),
            }
        }
    }

    /// A default semaphore and mutex implementation.
    ...
    impl CompatSemaphore {
        ...
        /// task.
        fn take_with_deadline(&self, deadline_instant: Option<u64>) -> bool {
            let deadline = deadline_instant.unwrap_or(u64::MAX);
            loop {
                let taken = self.inner.with(|sem| {
                    if sem.try_take() {
                        true
                    } else {
                        sem.wait_with_deadline(deadline_instant);
                        false
                    }
                });
        ...
        unsafe fn take(semaphore: SemaphorePtr, timeout_us: Option<u32>) -> bool {
            unsafe {
                <Self as SemaphoreImplementation>::take_with_deadline(
                    semaphore,
                    timeout_us.map(|timeout| now() + timeout as u64),
                )
            }
        }
