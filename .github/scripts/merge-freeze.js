// Merge freeze: `gate` enforces it, `recordExemptMerge` notes what slipped
// through.
//
// The state is an open issue labelled `merge-freeze`, opened and closed by
// hand. While it is open, merges into `main` are rejected unless the pull
// request carries `merge-freeze-exempt`.
...
// More than one issue may end up claiming to hold the freeze, so the oldest
// wins and a stray duplicate cannot take over. Pull requests are dropped
// because the API lists them alongside issues, and one carrying the label must
// not freeze anything.
...
module.exports = { gate, recordExemptMerge };
