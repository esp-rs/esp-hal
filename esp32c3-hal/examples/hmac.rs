...
use esp32c3_hal::{
    ...
    prelude::*,
    system::Peripheral as PeripheralEnable,
...
fn main() -> ! {
    ...
    // Set peripheral clock and clear the peripheral reset
    system
        .peripheral_clock_control
        .enable(PeripheralEnable::Sha);
    system
        .peripheral_clock_control
        .enable(PeripheralEnable::Hmac);
    ...
    for i in 0..src.len() + 1 {
        ...
        print!("ok");
        print!(
            " (it took {} cycles for hw hash",
            post_hw_hmac - pre_hw_hmac
        );
        if i > 0 {
            print!(
                " or {} cycles/byte",
                (post_hw_hmac - pre_hw_hmac) / (i as u64)
            );
        }
        println!(")");
    }
