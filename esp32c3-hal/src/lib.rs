...
pub use esp_hal_common::{
    clock, efuse, gpio as gpio_types, i2c, interrupt, pac, prelude, pulse_control, ram, serial,
    spi, system, systimer, timer, utils, Cpu, Delay, PulseControl, Rng, Serial, UsbSerialJtag,
};
...
unsafe fn configure_mmu() {
    ...
    for i in 0..FLASH_MMU_TABLE_SIZE {
        *(FLASH_MMU_TABLE.offset(i as isize)) = MMU_TABLE_INVALID_VAL;
    }
...
#[doc(hidden)]
#[cfg_attr(feature = "mcuboot", link_section = ".rwtext")]
pub fn mp_hook() -> bool {
    #[cfg(feature = "mcuboot")]
    unsafe {
        configure_mmu();
