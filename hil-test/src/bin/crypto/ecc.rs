...
#[cfg(any(ecc_has_point_addition, ecc_has_modular_arithmetic))]
struct TestData<const N: usize> {
    curve: EllipticCurve,
    mod_base: EccModBase,
    x: [u8; N],
    y: [u8; N],
...
#[cfg(all(
    any(ecc_has_point_addition, ecc_has_modular_arithmetic),
    ecc_has_curve_p192
))]
const P192_TEST_DATA: TestData<24> = TestData {
...
const P256_TEST_DATA: TestData<32> = TestData {
...
const P384_TEST_DATA: TestData<48> = TestData {
    ...
    num: [
        0x68, 0xd1, 0x09, 0xa7, 0xc7, 0x7e, 0xeb, 0xbd, 0x43, 0x18, 0x7e, 0xdd, 0x69, 0x23, 0x7e,
        0x0a, 0xef, 0x07, 0xc2, 0x0e, 0xc5, 0x3d, 0xe7, 0xcb, 0xd4, 0x36, 0xad, 0x9b, 0xdc, 0xf8,
        0x6c, 0x5c, 0x0c, 0x3d, 0xce, 0x45, 0xcd, 0x6f, 0x7f, 0x18, 0x40, 0xc5, 0x29, 0xf3, 0xcd,
        0x12, 0x1d, 0xc2,
    ],
    den: [
        0x68, 0xd1, 0x09, 0xa7, 0xc7, 0x7e, 0xeb, 0xbd, 0x43, 0x18, 0x7e, 0xdd, 0x69, 0x23, 0x7e,
        0x0a, 0xef, 0x07, 0xc2, 0x0e, 0xc5, 0x3d, 0xe7, 0xcb, 0xd4, 0x36, 0xad, 0x9b, 0xdc, 0xf8,
        0x6c, 0x5c, 0x0c, 0x3d, 0xce, 0x45, 0xcd, 0x6f, 0x7f, 0x18, 0x40, 0xc5, 0x8c, 0x56, 0x68,
        0xb7, 0xb8, 0x67,
    ],
    expected_sum: [
        0x6D, 0x75, 0xE3, 0xA0, 0xE9, 0x98, 0x45, 0xB9, 0x70, 0xA8, 0xAF, 0x95, 0xD3, 0xA5, 0x6F,
        0x46, 0x87, 0xE4, 0x21, 0x2B, 0x32, 0xF4, 0x4C, 0x4D, 0x43, 0xD2, 0x73, 0xF3, 0x37, 0x45,
        0xE3, 0xF8, 0x5F, 0x62, 0xA3, 0x2C, 0xDD, 0xD3, 0xAA, 0x09, 0xB5, 0x97, 0x7B, 0xB4, 0x02,
        0x61, 0x19, 0x16,
    ],
    expected_diff: [
        0x74, 0x70, 0xEC, 0xD7, 0x27, 0x65, 0xD9, 0xC7, 0x30, 0x13, 0x2F, 0x5F, 0x60, 0x8E, 0xD0,
        0x4A, 0x76, 0x28, 0x1D, 0xA5, 0x62, 0x0D, 0x87, 0x1C, 0x70, 0x1C, 0x10, 0xCD, 0xCD, 0x63,
        0x71, 0x77, 0x4A, 0xA2, 0x40, 0x8F, 0xA1, 0xD7, 0xA7, 0xCE, 0xBF, 0x10, 0x41, 0xBC, 0xE1,
        0x8B, 0xFB, 0x57,
    ],
    expected_prod: [
        0x63, 0x67, 0x7D, 0x8B, 0x32, 0x4C, 0x13, 0xE6, 0x49, 0xAB, 0xDE, 0x9F, 0xDB, 0x68, 0x57,
        0x49, 0xDE, 0x88, 0x77, 0x56, 0x45, 0xB0, 0x7B, 0xD7, 0xAB, 0xFB, 0xF4, 0x55, 0xC0, 0xD3,
        0xD0, 0x2D, 0x37, 0x14, 0x8F, 0x3A, 0x1E, 0x72, 0x7E, 0x49, 0x77, 0xA0, 0xB9, 0xC8, 0xD0,
        0x44, 0xDD, 0x16,
    ],
    expected_div: [
        0x05, 0x35, 0xe1, 0x77, 0xd0, 0xd0, 0x47, 0x38, 0x65, 0xe8, 0x4c, 0xeb, 0x31, 0x96, 0xd9,
        0xfc, 0x18, 0x0a, 0xe2, 0xd9, 0x5d, 0x40, 0xf4, 0x89, 0x8a, 0xc6, 0xfc, 0x32, 0x5f, 0xd8,
        0xc7, 0x74, 0x15, 0x44, 0xa5, 0xd6, 0xca, 0x72, 0xc1, 0x2f, 0xd7, 0xb3, 0x17, 0xda, 0x6f,
        0x49, 0x17, 0xb7,
    ],
};
...
fn affine_point_multiplication(curve: EllipticCurve, k: &[u8], sw_x: &mut [u8], sw_y: &mut [u8]) {
    for_each_ecc_curve! {
        (all $(($_id:tt, $name:tt, $bits:literal)),*) => {
            match curve {
                $(
...
mod tests {
    ...
    fn test_ecc_point_addition<const N: usize>(mut ctx: Context<'static>, test_case: TestData<N>) {
        let mut one = [0u8; N];
        one[0] = 1;
    ...
    fn test_ecc_point_addition_192(ctx: Context<'static>) {
        test_ecc_point_addition(ctx, P192_TEST_DATA);
    }

    #[test]
    #[cfg(ecc_has_point_addition)]
    fn test_ecc_point_addition_256(ctx: Context<'static>) {
        test_ecc_point_addition(ctx, P256_TEST_DATA);
    }

    #[test]
    #[cfg(all(ecc_has_point_addition, ecc_has_curve_p384))]
    fn test_ecc_point_addition_384(ctx: Context<'static>) {
        test_ecc_point_addition(ctx, P384_TEST_DATA);
    }

    #[cfg(ecc_has_modular_arithmetic)]
    fn run_mod_tests<const N: usize>(mut ctx: Context<'static>, test_case: TestData<N>) {
        let result = &mut [0; N][..];
...
mod work_queue_tests {
    ...
    fn init() -> Context<'static> {
        ...
        Context {
            ecc: EccBackend::new(p.ECC, Default::default()),
            #[cfg(rng_trng_supported)]
            _rng_source: TrngSource::new(p.RNG, p.ADC1),
        }
    }
    ...
    #[cfg(ecc_has_modular_arithmetic)]
    fn run_backend_mod_tests<const N: usize>(test_case: TestData<N>) {
        let mut result = [0; N];

        test_case
            .curve
            .modular_addition(test_case.mod_base, &test_case.x, &test_case.y)
            .and_then(|op| op.with_scalar_result(&mut result))
            .unwrap()
            .process()
            .wait_blocking();
        assert_eq!(result, test_case.expected_sum);
        ...
        test_case
            .curve
            .modular_subtraction(test_case.mod_base, &test_case.x, &test_case.y)
            .and_then(|op| op.with_scalar_result(&mut result))
            .unwrap()
            .process()
            .wait_blocking();
        ...
        test_case
            .curve
            .modular_multiplication(test_case.mod_base, &test_case.x, &test_case.y)
            .and_then(|op| op.with_scalar_result(&mut result))
            .unwrap()
            .process()
            .wait_blocking();
        ...
        test_case
            .curve
            .modular_division(test_case.mod_base, &test_case.num, &test_case.den)
            .and_then(|op| op.with_scalar_result(&mut result))
            .unwrap()
            .process()
            .wait_blocking();
    ...
    #[test]
    #[cfg(ecc_has_modular_arithmetic)]
    fn test_ecc_backend_mod_operations_192(mut ctx: Context<'static>) {
        let _backend = ctx.ecc.start();
        run_backend_mod_tests(P192_TEST_DATA);
    }

    #[test]
    #[cfg(ecc_has_modular_arithmetic)]
    fn test_ecc_backend_mod_operations_256(mut ctx: Context<'static>) {
        let _backend = ctx.ecc.start();
        run_backend_mod_tests(P256_TEST_DATA);
    }

    #[test]
    #[cfg(all(ecc_has_modular_arithmetic, ecc_has_curve_p384))]
    fn test_ecc_backend_mod_operations_384(mut ctx: Context<'static>) {
        let _backend = ctx.ecc.start();
        run_backend_mod_tests(P384_TEST_DATA);
    }
}
