...
fn setup<Dm: DriverMode + 'static>(
    tx_config: TxChannelConfig,
    rx_config: RxChannelConfig,
) -> (AnyTxChannel<'static, Dm>, AnyRxChannel<'static, Dm>)
where
...
mod tests {
    ...
    async fn rmt_async_tx_invalid_args_slice() {
        ...
        assert_eq!(
            ch0.transmit(&empty).await,
            Err(Error::InvalidArgument)
        );

        let code = PulseCode::default().with_length1(42).unwrap().with_length2(42).unwrap();
    ...
    async fn rmt_loopback_after_drop() {
        ...
        // ...then start over and check that everything still works as expected (i.e. we didn't
        // leave the hardware in an unexpected state or lock up when dropping the futures).
        let (rx_res, tx_res) = embassy_futures::join::join(
            rx_channel.receive(&mut rcv_data),
            tx_channel.transmit(&tx_data),
        )
        ...
        check_data_eq(&tx_data, &rcv_data, Some(rx_count), rx_channel.buffer_size());
    }
    ...
    async fn rmt_pin_reconfigure() {
        ...
        let ch0 = {
            let mut rmt = Rmt::new(peripherals.RMT.reborrow(), FREQ).unwrap().into_async();
        ...
        // Note that we can keep around the channel longer than the Rmt! This is intended and
        // useful when e.g. creating the Rmt in the main task and then sending several channels to
        // different places.
        let _ = ch0;
    }
}
