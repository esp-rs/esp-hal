...
mod tests {
    ...
    fn test_dma_rx_descriptors(mut ctx: Context) {
        ...
        ctx.spi.read(&mut test_rx).expect("error reading from SDIO host");
    ...
    fn test_dma_tx_descriptors(mut ctx: Context) {
        ...
        // enable the SLC1 interrupt, and set the handler
        interrupt::enable(Interrupt::SLC1, Priority::Priority1).expect("error enabling SLC1 interrupt"); 
        unsafe { interrupt::bind_interrupt(Interrupt::SLC1, IsrCallback::new(slc1_handler)) };

        // TODO: mock transfers from the SDIO host
        ctx.spi.write(&test_buffer).expect("error writing SDIO from host");
    }
}

extern "C" fn slc1_handler() {
