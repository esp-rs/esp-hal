...
mod _p4_peripheral_clocks {
    ...
    impl Peripheral {
        ...
        pub const KEEP_ENABLED: &[Peripheral] =
            &[Peripheral::Systimer, Peripheral::Iomux, Peripheral::UsbDevice];
        pub const COUNT: usize = Self::ALL.len();
        pub const ALL: &[Self] = &[
            Self::Uart0, Self::Uart1, Self::Uart2, Self::Uart3, Self::Uart4,
            Self::Spi2, Self::Spi3,
            Self::I2c0, Self::I2c1,
            Self::I2s0, Self::I2s1, Self::I2s2,
            Self::Systimer, Self::Timg0, Self::Timg1,
            Self::Iomux,
            Self::Ledc, Self::Pcnt, Self::Mcpwm0, Self::Mcpwm1, Self::Rmt,
            Self::Twai0, Self::Twai1, Self::Twai2,
            Self::UsbOtg11, Self::UsbOtg20, Self::UsbDevice,
            Self::Emac, Self::Sdmmc, Self::Uhci,
            Self::Aes, Self::Sha, Self::Rsa, Self::Ecc, Self::Ecdsa, Self::Hmac, Self::Ds,
            Self::Dma, Self::Gdma, Self::AhbPdma, Self::AxiPdma,
            Self::Adc, Self::Parlio, Self::LcdCam,
        ];
    }
    ...
    pub(crate) unsafe fn enable_internal_racey(peripheral: Peripheral, enable: bool) {
        ...
        match peripheral {
            ...
            Peripheral::Uart0 => {
                c.soc_clk_ctrl1().modify(|_, w| w.uart0_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uart0_apb_clk_en().bit(enable));
            }
            Peripheral::Uart1 => {
                c.soc_clk_ctrl1().modify(|_, w| w.uart1_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uart1_apb_clk_en().bit(enable));
            }
            Peripheral::Uart2 => {
                c.soc_clk_ctrl1().modify(|_, w| w.uart2_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uart2_apb_clk_en().bit(enable));
            }
            Peripheral::Uart3 => {
                c.soc_clk_ctrl1().modify(|_, w| w.uart3_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uart3_apb_clk_en().bit(enable));
            }
            Peripheral::Uart4 => {
                c.soc_clk_ctrl1().modify(|_, w| w.uart4_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uart4_apb_clk_en().bit(enable));
            }
            // -- SPI: sys_clk + apb_clk --
            Peripheral::Spi2 => {
                c.soc_clk_ctrl1().modify(|_, w| w.gpspi2_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.gpspi2_apb_clk_en().bit(enable));
            }
            Peripheral::Spi3 => {
                c.soc_clk_ctrl1().modify(|_, w| w.gpspi3_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.gpspi3_apb_clk_en().bit(enable));
            }
            // -- I2C: apb_clk only --
            Peripheral::I2c0 => {
                c.soc_clk_ctrl2().modify(|_, w| w.i2c0_apb_clk_en().bit(enable));
            }
            Peripheral::I2c1 => {
                c.soc_clk_ctrl2().modify(|_, w| w.i2c1_apb_clk_en().bit(enable));
            }
            // -- I2S: apb_clk --
            Peripheral::I2s0 => {
                c.soc_clk_ctrl2().modify(|_, w| w.i2s0_apb_clk_en().bit(enable));
            }
            Peripheral::I2s1 => {
                c.soc_clk_ctrl2().modify(|_, w| w.i2s1_apb_clk_en().bit(enable));
            }
            Peripheral::I2s2 => {
                c.soc_clk_ctrl2().modify(|_, w| w.i2s2_apb_clk_en().bit(enable));
            }
            // -- Timers --
            Peripheral::Systimer => {
                c.soc_clk_ctrl2().modify(|_, w| w.systimer_apb_clk_en().bit(enable));
                c.peri_clk_ctrl21().modify(|_, w| w.systimer_clk_en().bit(enable));
            }
            Peripheral::Timg0 => {
                c.soc_clk_ctrl2().modify(|_, w| w.timergrp0_apb_clk_en().bit(enable));
            }
            Peripheral::Timg1 => {
                c.soc_clk_ctrl2().modify(|_, w| w.timergrp1_apb_clk_en().bit(enable));
            }
            // -- GPIO/IOMUX --
            Peripheral::Iomux => {
                c.peri_clk_ctrl26().modify(|_, w| w.iomux_clk_en().bit(enable));
            }
            // -- PWM/Counter --
            Peripheral::Ledc => {
            ...
            Peripheral::Pcnt => {
                c.soc_clk_ctrl2().modify(|_, w| w.pcnt_apb_clk_en().bit(enable));
            }
            Peripheral::Mcpwm0 => {
                c.soc_clk_ctrl2().modify(|_, w| w.mcpwm0_apb_clk_en().bit(enable));
            }
            Peripheral::Mcpwm1 => {
                c.soc_clk_ctrl2().modify(|_, w| w.mcpwm1_apb_clk_en().bit(enable));
            }
            Peripheral::Rmt => {
                c.soc_clk_ctrl2().modify(|_, w| w.rmt_sys_clk_en().bit(enable));
            }
            // -- TWAI (CAN) --
            Peripheral::Twai0 => {
                c.soc_clk_ctrl2().modify(|_, w| w.twai0_apb_clk_en().bit(enable));
            }
            Peripheral::Twai1 => {
                c.soc_clk_ctrl2().modify(|_, w| w.twai1_apb_clk_en().bit(enable));
            }
            Peripheral::Twai2 => {
                c.soc_clk_ctrl2().modify(|_, w| w.twai2_apb_clk_en().bit(enable));
            }
            // -- USB --
            Peripheral::UsbOtg11 => {
                c.soc_clk_ctrl1().modify(|_, w| w.usb_otg11_sys_clk_en().bit(enable));
            }
            Peripheral::UsbOtg20 => {
                c.soc_clk_ctrl1().modify(|_, w| w.usb_otg20_sys_clk_en().bit(enable));
            }
            Peripheral::UsbDevice => {
                c.soc_clk_ctrl2().modify(|_, w| w.usb_device_apb_clk_en().bit(enable));
            }
            // -- Connectivity --
            Peripheral::Emac => {
                c.soc_clk_ctrl1().modify(|_, w| w.emac_sys_clk_en().bit(enable));
            }
            Peripheral::Sdmmc => {
                c.soc_clk_ctrl1().modify(|_, w| w.sdmmc_sys_clk_en().bit(enable));
            }
            Peripheral::Uhci => {
                c.soc_clk_ctrl1().modify(|_, w| w.uhci_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.uhci_apb_clk_en().bit(enable));
            }
            // -- Crypto (shared sys clock) --
            Peripheral::Aes | Peripheral::Sha | Peripheral::Rsa |
            Peripheral::Ecc | Peripheral::Ecdsa | Peripheral::Hmac | Peripheral::Ds => {
                c.soc_clk_ctrl1().modify(|_, w| w.crypto_sys_clk_en().bit(enable));
            }
            // -- DMA --
            // Dma = AHB_DMA (GDMA v2 compatible, used by esp-hal)
            Peripheral::Dma => {
                c.soc_clk_ctrl1().modify(|_, w| w.ahb_pdma_sys_clk_en().bit(enable));
            }
            // Gdma = DW_GDMA (DesignWare, NOT used by esp-hal DMA driver)
            Peripheral::Gdma => {
                c.soc_clk_ctrl1().modify(|_, w| w.gdma_sys_clk_en().bit(enable));
            }
            Peripheral::AhbPdma => {
                c.soc_clk_ctrl1().modify(|_, w| w.ahb_pdma_sys_clk_en().bit(enable));
            }
            Peripheral::AxiPdma => {
                c.soc_clk_ctrl1().modify(|_, w| w.axi_pdma_sys_clk_en().bit(enable));
            }
            // -- ADC --
            Peripheral::Adc => {
                c.soc_clk_ctrl2().modify(|_, w| w.adc_apb_clk_en().bit(enable));
            }
            // -- Parallel IO --
            Peripheral::Parlio => {
                c.soc_clk_ctrl1().modify(|_, w| w.parlio_sys_clk_en().bit(enable));
                c.soc_clk_ctrl2().modify(|_, w| w.parlio_apb_clk_en().bit(enable));
            }
            // -- LCD/Camera --
            Peripheral::LcdCam => {
    ...
    pub(crate) unsafe fn assert_peri_reset_racey(peripheral: Peripheral, reset: bool) {
        ...
        match peripheral {
            ...
            Peripheral::Uart0 => { c.hp_rst_en1().modify(|_, w| {
                w.rst_en_uart0_core().bit(reset).rst_en_uart0_apb().bit(reset)
            }); }
            Peripheral::Uart1 => { c.hp_rst_en1().modify(|_, w| {
                w.rst_en_uart1_core().bit(reset).rst_en_uart1_apb().bit(reset)
            }); }
            Peripheral::Uart2 => { c.hp_rst_en1().modify(|_, w| {
                w.rst_en_uart2_core().bit(reset).rst_en_uart2_apb().bit(reset)
            }); }
            Peripheral::Uart3 => { c.hp_rst_en1().modify(|_, w| {
                w.rst_en_uart3_core().bit(reset).rst_en_uart3_apb().bit(reset)
            }); }
            Peripheral::Uart4 => { c.hp_rst_en1().modify(|_, w| {
                w.rst_en_uart4_core().bit(reset).rst_en_uart4_apb().bit(reset)
            }); }
            // -- SPI: HP_RST_EN2 --
            Peripheral::Spi2 => { c.hp_rst_en2().modify(|_, w| w.rst_en_spi2().bit(reset)); }
            Peripheral::Spi3 => { c.hp_rst_en2().modify(|_, w| w.rst_en_spi3().bit(reset)); }
            // -- I2C: HP_RST_EN1 --
            Peripheral::I2c0 => { c.hp_rst_en1().modify(|_, w| w.rst_en_i2c0().bit(reset)); }
            Peripheral::I2c1 => { c.hp_rst_en1().modify(|_, w| w.rst_en_i2c1().bit(reset)); }
            // -- I2S: HP_RST_EN2 --
            Peripheral::I2s0 => { c.hp_rst_en2().modify(|_, w| w.rst_en_i2s0_apb().bit(reset)); }
            Peripheral::I2s1 => { c.hp_rst_en2().modify(|_, w| w.rst_en_i2s1_apb().bit(reset)); }
            Peripheral::I2s2 => { c.hp_rst_en2().modify(|_, w| w.rst_en_i2s2_apb().bit(reset)); }
            // -- Timers: HP_RST_EN1 --
            Peripheral::Systimer => { c.hp_rst_en1().modify(|_, w| w.rst_en_stimer().bit(reset)); }
            Peripheral::Timg0 => { c.hp_rst_en1().modify(|_, w| w.rst_en_timergrp0().bit(reset)); }
            Peripheral::Timg1 => { c.hp_rst_en1().modify(|_, w| w.rst_en_timergrp1().bit(reset)); }
            // -- IOMUX: HP_RST_EN1 --
            Peripheral::Iomux => { c.hp_rst_en1().modify(|_, w| w.rst_en_iomux().bit(reset)); }
            // -- PWM/Counter: HP_RST_EN1 --
            Peripheral::Ledc => { c.hp_rst_en1().modify(|_, w| w.rst_en_ledc().bit(reset)); }
            Peripheral::Pcnt => { c.hp_rst_en1().modify(|_, w| w.rst_en_pcnt().bit(reset)); }
            Peripheral::Mcpwm0 => { c.hp_rst_en1().modify(|_, w| w.rst_en_pwm0().bit(reset)); }
            Peripheral::Mcpwm1 => { c.hp_rst_en1().modify(|_, w| w.rst_en_pwm1().bit(reset)); }
            Peripheral::Rmt => { c.hp_rst_en1().modify(|_, w| w.rst_en_rmt().bit(reset)); }
            // -- TWAI: HP_RST_EN1 (named can0/1/2) --
            Peripheral::Twai0 => { c.hp_rst_en1().modify(|_, w| w.rst_en_can0().bit(reset)); }
            Peripheral::Twai1 => { c.hp_rst_en1().modify(|_, w| w.rst_en_can1().bit(reset)); }
            Peripheral::Twai2 => { c.hp_rst_en1().modify(|_, w| w.rst_en_can2().bit(reset)); }
            // -- USB: reset via LP domain registers, not HP_RST_EN --
            Peripheral::UsbOtg11 | Peripheral::UsbOtg20 | Peripheral::UsbDevice => { return; }
            // -- Connectivity --
            Peripheral::Emac => { return; } // EMAC reset via dedicated register, not HP_RST_EN
            Peripheral::Sdmmc => { return; } // SDMMC reset not in HP_RST_EN
            Peripheral::Uhci => { c.hp_rst_en1().modify(|_, w| w.rst_en_uhci().bit(reset)); }
            // -- Crypto: HP_RST_EN2 --
            Peripheral::Aes => { c.hp_rst_en2().modify(|_, w| w.rst_en_aes().bit(reset)); }
            Peripheral::Sha => { c.hp_rst_en2().modify(|_, w| w.rst_en_sha().bit(reset)); }
            Peripheral::Rsa => { c.hp_rst_en2().modify(|_, w| w.rst_en_rsa().bit(reset)); }
            Peripheral::Ecc => { c.hp_rst_en2().modify(|_, w| w.rst_en_ecc().bit(reset)); }
            Peripheral::Ecdsa => { c.hp_rst_en2().modify(|_, w| w.rst_en_ecdsa().bit(reset)); }
            Peripheral::Hmac => { c.hp_rst_en2().modify(|_, w| w.rst_en_hmac().bit(reset)); }
            Peripheral::Ds => { c.hp_rst_en2().modify(|_, w| w.rst_en_ds().bit(reset)); }
            // -- DMA: HP_RST_EN0/1 --
            Peripheral::Dma => { c.hp_rst_en1().modify(|_, w| w.rst_en_ahb_pdma().bit(reset)); }
            Peripheral::Gdma => { c.hp_rst_en0().modify(|_, w| w.rst_en_gdma().bit(reset)); }
            Peripheral::AhbPdma => { c.hp_rst_en1().modify(|_, w| w.rst_en_ahb_pdma().bit(reset)); }
            Peripheral::AxiPdma => { c.hp_rst_en1().modify(|_, w| w.rst_en_axi_pdma().bit(reset)); }
            // -- ADC: HP_RST_EN2 --
            Peripheral::Adc => { c.hp_rst_en2().modify(|_, w| w.rst_en_adc().bit(reset)); }
            // -- Parallel IO: HP_RST_EN2 --
            Peripheral::Parlio => { c.hp_rst_en2().modify(|_, w| {
                w.rst_en_parlio().bit(reset)
                    .rst_en_parlio_rx().bit(reset)
                    .rst_en_parlio_tx().bit(reset)
            }); }
            // -- LCD/Camera: HP_RST_EN2 --
            Peripheral::LcdCam => { c.hp_rst_en2().modify(|_, w| w.rst_en_lcdcam().bit(reset)); }
        }
    }
}
