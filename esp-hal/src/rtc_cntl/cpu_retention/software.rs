...
pub(crate) fn sleep_retained(buffer: *mut u8, enter_sleep: fn(), wait: fn() -> bool) -> bool {
    ...
    let regions = esp32c6::regions();
    let device_frame = unsafe {
        slice::from_raw_parts_mut(
            buffer.add(esp32c6::DEVICE_REGIONS_OFFSET) as *mut u32,
            esp32c6::DEVICE_REGION_WORDS,
        )
    };
    ...
    let non_critical =
        unsafe { buffer.add(esp32c6::NON_CRITICAL_FRAME_OFFSET) as *mut NonCriticalSleepFrame };
    save_non_critical(non_critical);

    // SAFETY: the buffer is installed and sized for this chip.
    let critical = unsafe { buffer.add(esp32c6::CRITICAL_FRAME_OFFSET) as *mut CriticalSleepFrame };
