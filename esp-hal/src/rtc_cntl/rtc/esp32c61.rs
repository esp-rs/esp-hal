...
const fn hp_retention_regdma_config(dir: u8, entry: u8) -> u8 {
    // ESP32-C61 uses the narrow (3-bit) backup mode field, like the C6:
    // `PMU_HP_RETENTION_REGDMA_CONFIG(dir, entry)`.
    (((dir) << 2) | (entry & 0x3)) & 0x7
}

const HP_CALI_DBIAS: u8 = 26;
const LP_CALI_DBIAS: u8 = 25;

const ICG_MODEM_CODE_SLEEP: u8 = 0;
const ICG_MODEM_CODE_MODEM: u8 = 1;
const ICG_MODEM_CODE_ACTIVE: u8 = 2;

// ESP32-C61 `dig_sysclk_sel`/`backup_clk_sel` use `SOC_CPU_CLK_SRC_*`:
