...
const fn hp_retention_regdma_config(dir: u8, entry: u8) -> u8 {
    // ESP32-C5 widens the backup mode field: `PMU_HP_RETENTION_REGDMA_CONFIG`
    // becomes `(((dir) << 4) | (entry & 0xf)) & 0x1f`.
    (((dir) << 4) | (entry & 0xf)) & 0x1f
}

const HP_CALI_DBIAS: u8 = 28;
const LP_CALI_DBIAS: u8 = 28;

const ICG_MODEM_CODE_SLEEP: u8 = 0;
const ICG_MODEM_CODE_MODEM: u8 = 1;
const ICG_MODEM_CODE_ACTIVE: u8 = 2;

// ESP32-C5 `dig_sysclk_sel`/`backup_clk_sel` use `PMU_CLK_SRC_VAL`:
// XTAL = 0, RC_FAST = 1, PLL_F160M = 2, PLL_F240M = 3.
const HP_SYSCLK_XTAL: u8 = 0;
const HP_SYSCLK_PLL: u8 = 2;
...
bitfield::bitfield! {
    ...
    pub u8,   hp_sleep2active_backup_mode          , set_hp_sleep2active_backup_mode          : 22, 18;
    pub u8,   hp_modem2active_backup_mode          , set_hp_modem2active_backup_mode          : 27, 23;
...
bitfield::bitfield! {
    ...
    pub u8,   hp_sleep2modem_backup_mode           , set_hp_sleep2modem_backup_mode           : 24, 20;
...
bitfield::bitfield! {
    ...
    pub u8,   hp_modem2sleep_backup_mode           , set_hp_modem2sleep_backup_mode           : 24, 20;
    pub u8,   hp_active2sleep_backup_mode          , set_hp_active2sleep_backup_mode          : 29, 25;
...
impl HpSystemInit {
    fn active() -> Self {
        ...
        retention.set_hp_sleep2active_backup_clk_sel(HP_SYSCLK_XTAL);
        retention.set_hp_modem2active_backup_clk_sel(HP_SYSCLK_PLL);
...
impl LpSystemInit {
    fn active() -> Self {
        ...
        analog_regulator0.set_dbias(LP_CALI_DBIAS);
...
pub(crate) fn init(config: &ClockConfig) {
    ...
    PCR::regs()
        .ctrl_32k_conf()
        .modify(|_, w| unsafe { w.fosc_tick_num().bits(31) });
}
