...
cfg_select! {
    ...
    any(esp32c2, esp32c3, esp32c5, esp32c6, esp32c61, esp32h2, esp32p4) => {
        use crate::gpio::LpPinWithResistors as RtcIoWakeupPinType;
    }
}
...
impl WakeTriggers {
    ...
    ///
    /// A source rejects a sleep request if the source is already asserted at the request. Without the
    /// rejection, the chip sleeps through the event that the caller wants to wake on. A chip cannot
    /// reject on every source that it can wake from, so the enabled sources pass through the mask of
    /// the sources that the chip can reject on. Deep sleep cannot be rejected.
