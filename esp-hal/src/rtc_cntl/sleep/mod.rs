...
/// let pin_0 = Input::new(peripherals.GPIO4, config).unwrap();
///
/// let reason =
///     reset_reason(Cpu::ProCpu).unwrap_or(SocResetReason::ChipPowerOn);
///
...
///
/// let config = InputConfig::default().with_pull(Pull::None);
/// let pin_4 = Input::new(peripherals.GPIO4, config).unwrap();
...
#[cfg_attr(
    any(esp32c3, esp32c2),
    doc = "let pin_0 = Input::new(peripherals.GPIO2, config).unwrap();"
)]
#[cfg_attr(any(esp32c3, esp32c2), doc = "let mut pin_1 = peripherals.GPIO3;")]
#[cfg_attr(
    esp32s3,
    doc = "let pin_0 = Input::new(peripherals.GPIO17, config).unwrap();"
)]
