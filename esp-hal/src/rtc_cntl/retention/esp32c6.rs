...
pub(super) const OPS: &[SysOp] = &[
    // PRI_0: system clock/reset.
    continuous!(PCR, [uart(0).conf()]..=[sram_power_conf()]),
    ...
    continuous!(IO_MUX, [pin_ctrl()]..=[gpio(30)]),
    continuous!(GPIO, [func_out_sel_cfg(0)], 35),
    continuous!(GPIO, [status_next()]..=[func_in_sel_cfg(124)]),
    continuous!(GPIO, [bt_select()]..=[pin(34)]),
...
pub(crate) const CACHE_REGIONS: [Region; 2] = [
    region!(EXTMEM, [l1_cache_ctrl()]),
    region!(EXTMEM, [l1_cache_wrap_around_ctrl()]),
];
