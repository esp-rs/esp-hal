...
pub mod lp_i2c {
    ...
    impl<'d> LpI2c {
        pub fn new(
            ...
            unsafe {
                ...
                lp_io.gpio6().modify(|_, w| w.lp_gpio6_mcu_sel().variant(1)); // TODO

                lp_io.gpio7().modify(|_, w| w.lp_gpio7_mcu_sel().variant(1));

                // Set output mode to Normal
                lp_io
                    .pin6()
                    .modify(|_, w| w.lp_gpio6_pad_driver().set_bit());
                // Enable output (writing to write-1-to-set register, then internally the
                // `GPIO_OUT_REG` will be set)
                lp_io
                    .out_enable_w1ts()
                    .write(|w| w.lp_gpio_enable_w1ts().bits(1 << 6));
                // Enable input
                lp_io.gpio6().modify(|_, w| w.lp_gpio6_fun_ie().set_bit());

                // Disable pulldown (enable internal weak pull-down)
                lp_io
                    .gpio6()
                    .modify(|_, w| w.lp_gpio6_fun_wpd().clear_bit());
                // Enable pullup
                lp_io.gpio6().modify(|_, w| w.lp_gpio6_fun_wpu().set_bit());

                // Same process for SCL pin
                lp_io
                    .pin7()
                    .modify(|_, w| w.lp_gpio7_pad_driver().set_bit());
                // Enable output (writing to write-1-to-set register, then internally the
                // `GPIO_OUT_REG` will be set)
                lp_io
                    .out_enable_w1ts()
                    .write(|w| w.lp_gpio_enable_w1ts().bits(1 << 7));
                // Enable input
                lp_io.gpio7().modify(|_, w| w.lp_gpio7_fun_ie().set_bit());
                // Disable pulldown (enable internal weak pull-down)
                lp_io
                    .gpio7()
                    .modify(|_, w| w.lp_gpio7_fun_wpd().clear_bit());
                // Enable pullup
                lp_io.gpio7().modify(|_, w| w.lp_gpio7_fun_wpu().set_bit());

                // Select LP I2C function for the SDA and SCL pins
                lp_io.gpio6().modify(|_, w| w.lp_gpio6_mcu_sel().bits(1));
                lp_io.gpio7().modify(|_, w| w.lp_gpio7_mcu_sel().bits(1));
            }
            ...
            println!("source_clk {:?}", source_clk);
            println!("bus_freq {:?}", bus_freq);

            println!("clkm_div {:?}", clkm_div);
            println!("sclk_freq {:?}", sclk_freq);
            println!("half_cycle {:?}", half_cycle);
 ...
 /* 
        ...
        fn master_write(&mut self, addr: u8, bytes: &[u8]) -> Result<(), Error> {
            ...
            println!("cmd_idx first {:?}", cmd_iterator.clone() as u8);

            self.add_cmd_lp(&mut cmd_iterator, Command::Start)?;

            println!("cmd_idx second {:?}", cmd_iterator.clone() as u8);
            ...
            self.add_cmd_lp(
                &mut cmd_iterator,
                Command::Write {
                    ack_exp: Ack::Ack, // TODO Nack vs Ack?????
                    ack_check_en: true,
                    length: 1 as u8,
                },
            )?;
...
/* 
        fn wait_for_completion(&self) -> Result<(), Error> {
            loop {
                ...
                //     break;
                // }
                const LP_I2C_NACK_INT_ST: u32 = 10;
                const LP_I2C_TRANS_COMPLETE_INT_ST_M: u32 = 7;

                // if interrupts & I2C_LL_INTR_MASK {
                //     if interrupts & LP_I2C_NACK_INT_ST {
                //         self.i2c
                //         .int_clr()
                //         .write(|w| unsafe { w.bits(I2C_LL_INTR_MASK) });
                //     return Err(Error::InvalidResponse);
                //     } else if interrupts & LP_I2C_TRANS_COMPLETE_INT_ST_M {
                //         self.i2c
                //         .int_ena()
                //         .write(|w| unsafe { w.bits(~I2C_LL_INTR_MASK) });
                //     self.i2c
...
/* 
        fn enable_interrupts(&self) {
            self.i2c
                .int_ena()
                .modify(|_, w| unsafe { w.bits(I2C_LL_INTR_MASK) });
        }
*/
/* 
