...
impl SystemTimer {
    ...
    pub fn create<'d>(
        ...
        target2_isr: Option<InterruptHandler>,
    ) -> SystemTimers<'d, crate::Blocking> {
        crate::into_ref!(p);

        if let Some(interrupt) = target0_isr {
            unsafe {
        ...
        if let Some(interrupt) = target1_isr {
            unsafe {
                crate::interrupt::bind_interrupt(
        ...
        if let Some(interrupt) = target2_isr {
            unsafe {
                crate::interrupt::bind_interrupt(
...
/// This gives access to the individual [Alarm] instances.
pub struct SystemTimers<'d, DM: crate::Mode> {
