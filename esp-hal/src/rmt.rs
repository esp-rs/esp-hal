    ...
    D::Item: Borrow<PulseCode>,
{
    ...
    pub fn wait(mut self) -> Result<(), Error> {
        ...
        STATE[self.channel.raw().channel() as usize]
            .store(RmtState::TxIdle as u8, Ordering::Relaxed);
...
pub trait RxChannelAsync {
    ...
    /// The length of sequence cannot exceed the size of the allocated RMT
    /// RAM.
    fn receive<'a, D, T>(&mut self, data: D) -> RmtRxFuture<'_, 'a, Self::Raw, D::IntoIter, T>
    where
        Self: Sized,
        D: IntoIterator<Item = &'a mut T>,
        D::IntoIter: Unpin,
