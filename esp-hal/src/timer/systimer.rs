...
impl<'d> SystemTimer<'d> {
    ...
    /// Sets the counter value immediately. If the unit is running, counting
    /// continues from the new value.
    ///
    /// Can load back sleep time recorded by the RTC timer via software after
    /// light sleep.
    ///
    /// # Safety
    ///
...
impl Alarm<'_> {
    ...
    /// Returns `true` if the comparator is enabled and generates interrupts based
    ...
    /// Sets the comparator mode to target or periodic.
    ...
    /// Returns the comparator mode (target or periodic).
    ...
    /// Sets how often the comparator generates an interrupt in periodic mode.
    ...
    /// Sets when the comparator generates an interrupt in target mode.
