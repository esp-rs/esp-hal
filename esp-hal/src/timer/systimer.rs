...
// u64 division as much as possible, we use the highest bit of the divisor to determine the division
// method. If the bit is 1, we take the lower bits and shift the timestamp by that amount. If the
// bit is 0, we divide by the divisor.
//
...
impl<'d> SystemTimer<'d> {
    ...
    pub(crate) fn init_timestamp_scaler() {
        ...
        let packed_rate_and_method = if systimer_rate.is_multiple_of(1_000_000) {
            ...
            // The rate is not a multiple of 1MHz, we need to scale it up to prevent precision loss.
    ...
    fn us_to_ticks(us: u64) -> u64 {
        ...
        match correction & (SHIFT_TIMESTAMP_FLAG | UNEVEN_DIVIDER_FLAG) {
            ...
            v if v == UNEVEN_DIVIDER_FLAG => {
                ...
                let divided = if UNEVEN_MULTIPLIER.is_power_of_two() {
                    us >> UNEVEN_MULTIPLIER.ilog2()
                } else {
                    us / UNEVEN_MULTIPLIER as u64
                };

                let multiplier = correction & UNEVEN_DIVIDER_MASK;
                divided * multiplier as u64
            }
