...
use crate::{
    private::Sealed,
    rtc_cntl::{RtcClock, SocResetReason, reset_reason},
    system::Cpu,
    time::Rate,
};
...
impl Clocks {
    ...
    fn measure_xtal_frequency() -> XtalClock {
        if let Some(clock) = const { Self::xtal_frequency_from_config() } {
            ...
            // Re-measure on powerup to make sure we're not stuck with an incorrect value set by a
            // fishy bootloader.
            let force_measure = matches!(
                reset_reason(Cpu::current()),
                None | Some(SocResetReason::ChipPowerOn)
            );

            let mhz = if force_measure {
                RtcClock::estimate_xtal_frequency()
            } else {
                // If the frequency has been stored, use that value. Otherwise, estimate using a
                // different clock.
                RtcClock::read_xtal_freq_mhz().unwrap_or_else(RtcClock::estimate_xtal_frequency)
            };
