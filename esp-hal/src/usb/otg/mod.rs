...
//!
//! Create a [`Usb`] with [`Usb::new_fs`] (D+ / D− pins + any [`UsbFsInstance`], e.g.
//! [`peripherals::USB_FS`](crate::peripherals::USB_FS)) or [`Usb::new_hs`] ([`UsbHsInstance`], e.g.
//! [`peripherals::USB_HS`](crate::peripherals::USB_HS)). The handle is type-erased; per-controller
//! data lives in [`UsbOtgInfo`].
//!
...
use embassy_usb_synopsys_otg::{
    ...
    PhyType,
    State,
    host::{HostState, OtgHostState, on_host_interrupt},
    on_interrupt,
    otg_v1::Otg,
};
...
impl AnyUsb<'_> {
    delegate::delegate! {
        to match self {
            ...
            fn embassy_device_state(&self) -> OtgState<'static>;
            fn embassy_host_state(&self) -> OtgHostState<'static>;
        }
    }
}
...
impl<'d> USB_FS<'d> {
    ...
    fn device_state() -> &'static State<7> {
        static DEVICE: State<7> = State::new();
        &DEVICE
    }

    fn host_state() -> &'static HostState<8> {
        static HOST: HostState<8> = HostState::new();
        &HOST
    }
    ...
    fn info(&self) -> &'static UsbOtgInfo {
        ...
        pub(crate) fn otg_fs_device_interrupt() {
            let state = USB_FS::device_state().as_otg_state();
            unsafe {
                on_interrupt(
                    Otg::from_ptr(USB_FS::PTR.cast_mut().cast::<()>()),
                    &state,
        ...
        pub(crate) fn otg_fs_host_interrupt() {
            let state = USB_FS::host_state().as_otg_host_state();
            unsafe {
                on_host_interrupt(
                    Otg::from_ptr(USB_FS::PTR.cast_mut().cast::<()>()),
                    &state,
    ...
    fn embassy_device_state(&self) -> OtgState<'static> {
        Self::device_state().as_otg_state()
    }

    fn embassy_host_state(&self) -> OtgHostState<'static> {
        Self::host_state().as_otg_host_state()
    }
}
...
impl<'d> USB_HS<'d> {
    ...
    fn device_state() -> &'static State<16> {
        static DEVICE: State<16> = State::new();
        &DEVICE
    }

    fn host_state() -> &'static HostState<16> {
        static HOST: HostState<16> = HostState::new();
        &HOST
    }
    ...
    fn info(&self) -> &'static UsbOtgInfo {
        ...
        pub(crate) fn otg_hs_device_interrupt() {
            let state = USB_HS::device_state().as_otg_state();
            unsafe {
                on_interrupt(
                    Otg::from_ptr(USB_HS::PTR.cast_mut().cast::<()>()),
                    &state,
                    state.endpoint_count(),
                )
            }
        ...
        pub(crate) fn otg_hs_host_interrupt() {
            let state = USB_HS::host_state().as_otg_host_state();
            unsafe {
                on_host_interrupt(
                    Otg::from_ptr(USB_HS::PTR.cast_mut().cast::<()>()),
                    &state,
...
impl<'d> Usb<'d> {
    ...
    pub(crate) fn embassy_device_state(&self) -> OtgState<'static> {
        self.inner.embassy_device_state()
    }

    pub(crate) fn embassy_host_state(&self) -> OtgHostState<'static> {
