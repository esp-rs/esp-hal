...
unsafe fn encoder_write<'a, E: Encoder + 'a>(
    ...
    if writer.ptr < writer.end || done.is_break() {
        ...
        // Do not check for end markers in the inner loop in Encoder::encode() since this would
        // substantially increase the instruction count there. Instead, only check the last code to
        // report on error.
            ...
            if writer.ptr < writer.end {
                ...
                // eventually stop and return an error. Thus, to avoid the logic to implement this,
                // simply modify the last code that was written to contain an end marker.
                unsafe {
                    writer
                        .ptr
                        .sub(1)
