...
//! [Config::with_endianness], [Config::with_ws_width], etc.
//!
...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum WsWidth {
    /// World select signal will be kept active for half of the frame
    #[default]
    HalfFrame,
    /// World select signal will be kept active for the length of the first channel (PCM long frame
    /// standard)
    #[cfg(not(any(esp32, esp32s2)))]
    OneChannel,
    /// World select signal will be kept active for a single BCLK cycle (PCM short frame standard)
    Bit,
    /// World select signal will be kept active for the specified amount of bits(BCLK cycles)
    #[cfg(not(any(esp32, esp32s2)))]
    Bits(u16),
}
...
mod private {
    ...
    pub trait RegisterAccessPrivate: Signals + RegBlock {
        ...
        fn configure(&self, config: &Config) -> Result<(), ConfigError> {
            ...
            let tx_chan_mod = if config.tx_config.channels == Channels::STEREO {
                0
            } else if config.tx_config.channels == Channels::LEFT {
                3
            } else if config.tx_config.channels == Channels::RIGHT {
                4
            } else if config.tx_config.channels == Channels::MONO {
                0
            } else {
                unreachable!()
            };

            let rx_chan_mod = if config.rx_config.channels == Channels::STEREO {
                0
            } else if config.rx_config.channels == Channels::LEFT {
                3
            } else if config.rx_config.channels == Channels::RIGHT {
                4
            } else if config.rx_config.channels == Channels::MONO {
                0
            } else {
                unreachable!()
            };
