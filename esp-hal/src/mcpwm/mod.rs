...
struct PwmClockGuard;

impl PwmClockGuard {
    pub fn new() -> Self {
        ClockTree::with(clocks::request_mcpwm_clk);
        Self
    }
}

impl Drop for PwmClockGuard {
    fn drop(&mut self) {
        ClockTree::with(clocks::release_mcpwm_clk);
    }
}
...
impl PeripheralClockConfig {
    fn source_clock() -> Rate {
        ClockTree::with(|clocks| Rate::from_hz(clocks::mcpwm_clk_frequency(clocks)))
    }
