    ...
    Dm: DriverMode,
{
    ...
    pub fn apply_config(&mut self, config: &TxConfig) -> Result<(), ConfigError> {
        ...
        let frequency = ClockTree::with(crate::soc::clocks::parl_io_tx_clock_frequency);
    ...
    Dm: DriverMode,
{
    ...
    pub fn apply_config(&mut self, config: &RxConfig) -> Result<(), ConfigError> {
        ...
        let frequency = ClockTree::with(crate::soc::clocks::parl_io_rx_clock_frequency);
        let divider = frequency / config.frequency.as_hz();
        if divider > 0xffff {
            return Err(ConfigError::UnreachableClockRate);
        }
...
impl ParlIoTxGuard {
    pub fn new(_guard: GenericPeripheralGuard<{ Peripheral::ParlIo as u8 }>) -> Self {
        ClockTree::with(|clocks| {
            crate::soc::clocks::configure_parl_io_tx_clock(clocks, Default::default());
            crate::soc::clocks::request_parl_io_tx_clock(clocks);
        });
        Self { _guard }
    }
}
...
impl Drop for ParlIoTxGuard {
    fn drop(&mut self) {
        ClockTree::with(crate::soc::clocks::release_parl_io_tx_clock);
    }
}
...
impl ParlIoRxGuard {
    pub fn new(_guard: GenericPeripheralGuard<{ Peripheral::ParlIo as u8 }>) -> Self {
        ClockTree::with(|clocks| {
            crate::soc::clocks::configure_parl_io_rx_clock(clocks, Default::default());
            crate::soc::clocks::request_parl_io_rx_clock(clocks);
        });
        Self { _guard }
    }
}
...
impl Drop for ParlIoRxGuard {
    fn drop(&mut self) {
        ClockTree::with(crate::soc::clocks::release_parl_io_rx_clock);
    }
}
