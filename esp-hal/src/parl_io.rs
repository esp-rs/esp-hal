...
// TODO: C5 has the option for unlimited DMA transfers
const MAX_DMA_SIZE: usize = 65535;

/// Interrupt sources of the PARL_IO peripheral.
///
/// The [`ParlIoTx`] and [`ParlIoRx`] drivers expose the sources of their own
/// unit via [`ParlIoTxInterrupt`] and [`ParlIoRxInterrupt`].
...
/// Interrupt sources of the PARL_IO TX unit.
///
/// These sources fire on the `PARL_IO` interrupt, which is bound via
/// [`ParlIo::set_interrupt_handler`].
#[derive(Debug, EnumSetType)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum ParlIoTxInterrupt {
    /// Triggered when TX FIFO is empty. This interrupt indicates that there
    /// might be error in the data sent by TX.
    FifoReEmpty,
    /// Triggered when TX finishes sending a complete frame of data.
...
/// Interrupt sources of the PARL_IO RX unit.
///
/// These sources fire on the `PARL_IO` interrupt, which is bound via
/// [`ParlIo::set_interrupt_handler`].
#[derive(Debug, EnumSetType)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum ParlIoRxInterrupt {
    /// Triggered when RX FIFO is full. This interrupt indicates that there
    /// might be error in the data received by RX.
    FifoWOvf,
}
...
///
/// This is total: every [`ParlIoTxInterrupt`] source has a
/// [`ParlIoInterrupt`] counterpart.
impl From<ParlIoTxInterrupt> for ParlIoInterrupt {
    fn from(value: ParlIoTxInterrupt) -> Self {
        match value {
            ParlIoTxInterrupt::FifoReEmpty => ParlIoInterrupt::TxFifoReEmpty,
            ParlIoTxInterrupt::Eof => ParlIoInterrupt::TxEof,
        }
    }
}

/// Lossy reverse mapping: RX-only [`ParlIoInterrupt`] variants have no
/// [`ParlIoTxInterrupt`] counterpart and are dropped.
impl TryFrom<ParlIoInterrupt> for ParlIoTxInterrupt {
    type Error = ();

    fn try_from(value: ParlIoInterrupt) -> Result<Self, ()> {
        match value {
            ParlIoInterrupt::TxFifoReEmpty => Ok(ParlIoTxInterrupt::FifoReEmpty),
            ParlIoInterrupt::TxEof => Ok(ParlIoTxInterrupt::Eof),
            ParlIoInterrupt::RxFifoWOvf => Err(()),
        }
    }
}
...
/// Variant-for-variant mapping onto the peripheral's interrupt sources.
///
/// This is total: every [`ParlIoRxInterrupt`] source has a
/// [`ParlIoInterrupt`] counterpart.
impl From<ParlIoRxInterrupt> for ParlIoInterrupt {
    fn from(value: ParlIoRxInterrupt) -> Self {
        match value {
            ParlIoRxInterrupt::FifoWOvf => ParlIoInterrupt::RxFifoWOvf,
        }
    }
}

/// Lossy reverse mapping: TX-only [`ParlIoInterrupt`] variants have no
/// [`ParlIoRxInterrupt`] counterpart and are dropped.
impl TryFrom<ParlIoInterrupt> for ParlIoRxInterrupt {
    type Error = ();

    fn try_from(value: ParlIoInterrupt) -> Result<Self, ()> {
        match value {
            ParlIoInterrupt::RxFifoWOvf => Ok(ParlIoRxInterrupt::FifoWOvf),
            _ => Err(()),
        }
    }
}
...
/// Selects the source of the TX EOF event.
///
/// Only available on chips with `PARL_IO` version 2 (e.g. the ESP32-C5).
#[cfg(parl_io_version = "2")]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
#[non_exhaustive]
pub enum TxEofSource {
    /// The TX EOF event is generated once the configured number of bits has
    /// been transmitted (reset default).
    BitLen,
    /// The TX EOF event is generated by the DMA channel's EOF signal.
    DmaEof,
}
...
impl<'d> ParlIo<'d, Blocking> {
    ...
    /// Maps the TX sources of the given set onto the peripheral's interrupt
    /// sources.
    fn map_tx_to_parl_io(interrupts: EnumSet<ParlIoTxInterrupt>) -> EnumSet<ParlIoInterrupt> {
        let mut sources = EnumSet::new();
        for interrupt in interrupts {
            sources.insert(ParlIoInterrupt::from(interrupt));
        }
        sources
    }

    /// Maps the peripheral's asserted interrupt sources back to
    /// [`ParlIoTxInterrupt`].
    ///
    /// The mapping is lossy by design: RX-only [`ParlIoInterrupt`] variants
    /// have no [`ParlIoTxInterrupt`] counterpart and are dropped.
    fn map_parl_io_to_tx(sources: EnumSet<ParlIoInterrupt>) -> EnumSet<ParlIoTxInterrupt> {
        let mut interrupts = EnumSet::new();
        for source in sources {
            if let Ok(interrupt) = ParlIoTxInterrupt::try_from(source) {
    ...
    /// Maps the RX sources of the given set onto the peripheral's interrupt
    /// sources.
    fn map_rx_to_parl_io(interrupts: EnumSet<ParlIoRxInterrupt>) -> EnumSet<ParlIoInterrupt> {
        let mut sources = EnumSet::new();
        for interrupt in interrupts {
            sources.insert(ParlIoInterrupt::from(interrupt));
        }
    ...
    /// [`ParlIoRxInterrupt`].
    ///
    /// The mapping is lossy by design: TX-only [`ParlIoInterrupt`] variants
    /// have no [`ParlIoRxInterrupt`] counterpart and are dropped.
    fn map_parl_io_to_rx(sources: EnumSet<ParlIoInterrupt>) -> EnumSet<ParlIoRxInterrupt> {
        let mut interrupts = EnumSet::new();
        for source in sources {
            if let Ok(interrupt) = ParlIoRxInterrupt::try_from(source) {
                interrupts.insert(interrupt);
            }
        }
        interrupts
    ...
    Dm: DriverMode,
{
    ...
    pub fn apply_config(&mut self, config: &TxConfig) -> Result<(), ConfigError> {
        ...
        #[cfg(parl_io_version = "2")]
        Instance::set_tx_eof_gen_sel(config.eof_source);
...
impl<'d> ParlIoTx<'d, Blocking> {
    /// Listens for the given PARL_IO TX interrupt sources.
    ///
    /// The handler serving these sources must have been registered via
    /// [`ParlIo::set_interrupt_handler`].
    pub fn listen(&mut self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_listen(ParlIo::map_tx_to_parl_io(interrupts.into()), true);
    }

    /// Stops listening for the given PARL_IO TX interrupt sources.
    pub fn unlisten(&mut self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_listen(ParlIo::map_tx_to_parl_io(interrupts.into()), false);
    }

    /// Returns the asserted PARL_IO TX interrupt sources.
    pub fn interrupts(&mut self) -> EnumSet<ParlIoTxInterrupt> {
        ParlIo::map_parl_io_to_tx(internal_interrupts())
    }

    /// Clears the given asserted PARL_IO TX interrupt sources.
    pub fn clear_interrupts(&mut self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_clear_interrupts(ParlIo::map_tx_to_parl_io(interrupts.into()));
    }
...
impl<'d> ParlIoRx<'d, Blocking> {
    /// Listens for the given PARL_IO RX interrupt sources.
    ///
    /// The handler serving these sources must have been registered via
    /// [`ParlIo::set_interrupt_handler`].
    pub fn listen(&mut self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_listen(ParlIo::map_rx_to_parl_io(interrupts.into()), true);
    }

    /// Stops listening for the given PARL_IO RX interrupt sources.
    pub fn unlisten(&mut self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_listen(ParlIo::map_rx_to_parl_io(interrupts.into()), false);
    }

    /// Returns the asserted PARL_IO RX interrupt sources.
    pub fn interrupts(&mut self) -> EnumSet<ParlIoRxInterrupt> {
        ParlIo::map_parl_io_to_rx(internal_interrupts())
    }

    /// Clears the given asserted PARL_IO RX interrupt sources.
    pub fn clear_interrupts(&mut self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_clear_interrupts(ParlIo::map_rx_to_parl_io(interrupts.into()));
    }
...
{
    /// Listens for the given PARL_IO TX interrupt sources.
    pub fn listen(&self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_listen(ParlIo::map_tx_to_parl_io(interrupts.into()), true);
    }

    /// Stops listening for the given PARL_IO TX interrupt sources.
    pub fn unlisten(&self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_listen(ParlIo::map_tx_to_parl_io(interrupts.into()), false);
    }

    /// Returns the asserted PARL_IO TX interrupt sources.
    pub fn interrupts(&self) -> EnumSet<ParlIoTxInterrupt> {
        ParlIo::map_parl_io_to_tx(internal_interrupts())
    }

    /// Clears the interrupt flags of the given PARL_IO TX interrupt sources.
    pub fn clear_interrupts(&self, interrupts: impl Into<EnumSet<ParlIoTxInterrupt>>) {
        internal_clear_interrupts(ParlIo::map_tx_to_parl_io(interrupts.into()));
    }
...
{
    /// Listens for the given PARL_IO RX interrupt sources.
    pub fn listen(&self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_listen(ParlIo::map_rx_to_parl_io(interrupts.into()), true);
    }

    /// Stops listening for the given PARL_IO RX interrupt sources.
    pub fn unlisten(&self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_listen(ParlIo::map_rx_to_parl_io(interrupts.into()), false);
    }

    /// Returns the asserted PARL_IO RX interrupt sources.
    pub fn interrupts(&self) -> EnumSet<ParlIoRxInterrupt> {
        ParlIo::map_parl_io_to_rx(internal_interrupts())
    }

    /// Clears the interrupt flags of the given PARL_IO RX interrupt sources.
    pub fn clear_interrupts(&self, interrupts: impl Into<EnumSet<ParlIoRxInterrupt>>) {
        internal_clear_interrupts(ParlIo::map_rx_to_parl_io(interrupts.into()));
    }
...
mod private {
    ...
    // TODO: these need to be metadata
    #[cfg(parl_io_version = "1")]
    ...
    impl Instance {
        ...
        pub fn set_tx_eof_gen_sel(value: TxEofSource) {
            let reg_block = PARL_IO::regs();

            reg_block
                .tx_genrl_cfg()
                .modify(|_, w| w.tx_eof_gen_sel().bit(matches!(value, TxEofSource::DmaEof)));
        }
