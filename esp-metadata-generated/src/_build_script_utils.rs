    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => {
                Err(
                    alloc::format!(
                        "Unknown chip {s}. Possible options: esp32, esp32c2, esp32c3, esp32c5, esp32c6, esp32h2, esp32s2, esp32s3"
                    ),
                )
            }
        }
    ...
    pub fn from_cargo_feature() -> Result<Self, &'static str> {
        ...
        for (env, c) in all_chips {
            if std::env::var(env).is_ok() {
                if chip.is_some() {
                    return Err(
                        "Expected exactly one of the following features to be enabled: esp32, esp32c2, esp32c3, esp32c5, esp32c6, esp32h2, esp32s2, esp32s3",
                    );
                }
                chip = Some(c);
            }
...
pub fn emit_check_cfg_directives() {
    ...
    println!(
        "cargo:rustc-check-cfg=cfg(gpio_constant_1_input, values(\"56\",\"30\",\"64\"))"
    );
    ...
    println!(
        "cargo:rustc-check-cfg=cfg(interrupt_controller, values(\"xtensa\",\"riscv_basic\",\"clic\",\"plic\"))"
    );
    println!(
    ...
    println!(
        "cargo:rustc-check-cfg=cfg(soc_cpu_csr_prv_mode, values(\"2064\",\"3088\"))"
    );
}
