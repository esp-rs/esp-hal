...
unsafe extern "C" fn ble_npl_time_ticks_to_ms(
    ...
    trace!("ble_npl_time_ms_to_ticks {}", time);
    unsafe {
        *p_ms = time;
    }
    0
...
unsafe extern "C" fn ble_npl_eventq_get(
    ...
    let mut event: *mut ble_npl_event = core::ptr::null_mut();
    loop {
        while queue.try_dequeue(addr_of_mut!(event).cast()) {
            let evt = unsafe { unwrap!(((*event).dummy as *mut Event).as_mut()) };
            if evt.queued {
...
unsafe extern "C" fn ble_npl_callout_init(
    ...
    if callout.dummy == 0 {
        let mut new_callout = Box::<Callout, _>::new_uninit_in(InternalMemory);
