...
/// or a conditional.
///
/// A replacement does not have to be a literal: anything that expands to a string literal works,
/// such as a call to a macro generated by esp-metadata. Lines containing such a replacement are
/// assembled with `concat!`, which means the value only needs to be valid for the configuration the
