...
impl SchedulerState {
    ...
    pub(crate) fn create_task(
        ...
        let mut task = Box::new_in(
            Task::new(task, param, task_stack_size, priority, pinned_to),
            InternalMemory,
        );
    ...
    fn run_scheduler(&mut self, task_switch: impl FnOnce(*mut CpuContext, *mut CpuContext)) {
        ...
        if next_task != self.per_cpu[current_cpu].current_task {
            ...
            let next_context = if let Some(mut next) = next_task {
                ...
                cfg_if::cfg_if! {
                    if #[cfg(xtensa)] {
                        self.per_cpu[current_cpu].idle_context.A1 = self.per_cpu[current_cpu].main_task.cpu_context.A1;
                    } else {
                        self.per_cpu[current_cpu].idle_context.sp = self.per_cpu[current_cpu].main_task.cpu_context.sp;
                    }
                }
                ...
                cfg_if::cfg_if! {
                    if #[cfg(xtensa)] {
                        self.per_cpu[current_cpu].idle_context.A1 = esp_hal::xtensa_lx::get_stack_pointer() as u32;
                    } else {
                        let current_sp;
                        unsafe { core::arch::asm!("mv {0}, sp", out(reg) current_sp); }
                        self.per_cpu[current_cpu].idle_context.sp = current_sp;
                    }
                }
                &raw mut self.per_cpu[current_cpu].idle_context
            };
...
impl Scheduler {
    ...
    pub(crate) fn create_task(
        ...
        self.with(|state| {
            state.create_task(task, param, task_stack_size, priority as usize, pinned_to)
        })
    }
