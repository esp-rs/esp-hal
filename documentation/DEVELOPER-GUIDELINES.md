...
- If a driver supports multiple peripheral instances (for example, I2C0 is one such instance):
  ...
  - The driver must implement a `new` constructor that automatically converts the peripheral instance into the any type. This is usually written as `fn new(peri: impl Instance + Into<AnyInstance<'d>>)`.
