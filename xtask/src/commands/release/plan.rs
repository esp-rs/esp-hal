...
//
// This file is automatically generated and meant to be manually edited. Change `new_version`
...
//
// Versions are decided here, at plan time. `new_version` already accounts for version numbers
// crates.io has permanently reserved (a yanked release keeps its number forever), and
// `execute-plan` writes it verbatim. To release a specific version, edit `new_version` directly
// -- editing `bump` alone will not change it. The release plan will then be added to the release
// PR and will be used to orchestrate the actual publishing process.
//
// For each package, this plan also records the bump `new_version` was derived from. It is kept
// for the changelog and migration-guide handling, and is what a re-run of `plan` would recompute
// the version from. A bump has two orthogonal fields — `base` (how much to bump
