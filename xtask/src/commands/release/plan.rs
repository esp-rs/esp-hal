...
//
// `new_version` is final: it skips version numbers crates.io already holds, and `execute-plan`
// writes it verbatim. To release a different version, edit `new_version` — editing `bump` alone
// will not change it. The release plan will then be added to the release PR and will be used to
// orchestrate the actual publishing process.
//
// For each package, this plan also records the bump `new_version` was derived from, which the
// changelog and migration-guide handling reads. A bump has two orthogonal fields — `base` (how
// much to bump major.minor.patch) and `pre` (the pre-release identifier):
//
