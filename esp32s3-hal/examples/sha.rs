...
fn main() -> ! {
    ...
    let mut sw_hasher_sha512_224 = sha2::Sha512_224::new();
    let mut sw_hasher_sha512_256 = sha2::Sha512_256::new();
    ...
    for mode in shamode {
        ...
        for i in 0..src.len() + 1 {
            ...
            match mode {
                ...
                ShaMode::SHA512_224 => {
                    sw_hasher_sha512_224.update(nsrc);
                    let soft_result = sw_hasher_sha512_224.finalize_reset();
                    for (a, b) in output.iter().zip(soft_result) {
                        assert_eq!(
                            *a, b,
                            "Sha512/224 failed during the {:?}th test with {:02X?} not equal to {:02X?}",
                            i, *a, b
                        );
                    }
                }
                ShaMode::SHA512_256 => {
                    sw_hasher_sha512_256.update(nsrc);
                    let soft_result = sw_hasher_sha512_256.finalize_reset();
                    for (a, b) in output.iter().zip(soft_result) {
                        assert_eq!(
                            *a, b,
                            "Sha512/256 failed during the {:?}th test with {:02X?} not equal to {:02X?}",
                            i, *a, b
                        );
                    }
                }
